...
public class DocumentResource extends BaseResource {
     ...
     * @apiParam {String} [search[tag]] The document must contain a tag matching the value, or a child of such a tag, case is ignored; the value is matched with the same rules as the <code>tag:</code> criteria of the search syntax, including the trailing <code>*</code> prefix operator (see "Document search syntax" on the top of the page), several comma-separated values can be specified and the document must match all tag filters
     * @apiParam {String} [search[nottag]] The document must not contain a tag matching the value, or a child of such a tag, case is ignored; the value is matched with the same rules as the <code>tag:</code> criteria of the search syntax, including the trailing <code>*</code> prefix operator (see "Document search syntax" on the top of the page), several comma-separated values can be specified and the document must match all tag filters
