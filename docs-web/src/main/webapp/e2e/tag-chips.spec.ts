...
test('clicking a tag chip in the document view filters the list by that tag (#34)', async ({ page, request, cleanup }) => {
  ...
  cleanup.defer('delete the chip tag', () => deleteTagApi(request, tagId))
  ...
  const taggedId = await apiCreateDocument(request, taggedTitle, [tagId])
  cleanup.defer('purge the tagged document', () => deleteDocApi(request, taggedId))
  const untaggedId = await apiCreateDocument(request, untaggedTitle, [])
