...
test('deleting a user reassigns their document to a chosen target, whose file still opens', async ({
  ...
  const departingContext = await browser.newContext({ storageState: { cookies: [], origins: [] } })
  // The body closes this context itself once the upload is done (step 3 needs the account
  // gone); `close()` is idempotent, so this only fires when the body did not get that far.
  cleanup.defer('close the departing user context', () => departingContext.close())
  ...
  const docId = departingPage.url().split('/document/view/')[1].split(/[/?#]/)[0]
  // Cleanup the reassigned document (owned by admin once step 3 has run).
  cleanup.defer('purge the reassigned document', () => deleteDocApi(page.request, docId))
