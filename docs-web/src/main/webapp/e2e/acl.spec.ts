...
test('grant READ on a document to a second user, see it listed, then revoke it', async ({ page, cleanup }) => {
  ...
  // Each teardown is registered the moment its entity exists, so a mid-test failure
  // still tears down whatever was created (what the old try/finally provided).
  ...
  const id = (await createDocument(page, unique('acl-doc'))).id
