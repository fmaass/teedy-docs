...
test('non-admin is redirected away from admin routes but can reach account settings', async ({ page, browser, cleanup }) => {
...
test('an admin can disable a user (login denied) and re-enable them (login restored)', async ({ page, browser, cleanup }) => {
...
test('the disable/enable toggle is hidden for the guest and admin rows', async ({ page, cleanup }) => {
