...
test('the "Assigned to me" filter round-trips through open + in-app Back (#28)', async ({ page, request, cleanup }) => {
  ...
  // Cleanup via API, documents first (deleting one cancels its route steps server-side)
  // and the route model last.
  const assignedId = await apiCreateDocument(request, assignedTitle)
  cleanup.defer('purge the assigned document', () => deleteDocApi(page.request, assignedId))
  const otherId = await apiCreateDocument(request, otherTitle)
  cleanup.defer('purge the unassigned document', () => deleteDocApi(page.request, otherId))
  const modelId = await apiCreateRouteModel(request, modelName)
  cleanup.defer('delete the seeded route model', () => request.delete(`/api/routemodel/${modelId}`))
