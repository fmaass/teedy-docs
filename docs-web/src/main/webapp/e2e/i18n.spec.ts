...
// end (and in a deferred cleanup step) so the shared admin storageState does not leak
// German into later specs.
...
test('switching the UI language to German and back updates and persists rendered strings', async ({ page, cleanup }) => {
  ...
  // unreachable on about:blank) and before the switch it undoes.
  cleanup.defer('restore the English UI locale for later specs', () =>
    page.evaluate(() => localStorage.setItem('teedy-locale', 'en')),
  )
