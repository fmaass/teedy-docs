...
test('star from list, see it on detail, filter by favorites, unstar (#41)', async ({ page, request, cleanup }) => {
  const favedTitle = unique('fav-doc')
  const otherTitle = unique('fav-other')

  const favedId = await apiCreateDocument(request, favedTitle)
  cleanup.defer('purge the favorited document', () => deleteDocApi(page.request, favedId))
