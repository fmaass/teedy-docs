...
test("a document's activity tab shows audit entries scoped to that document", async ({ page, cleanup }) => {
...
test('#113 the activity date renders at the same type scale as the rest of the row', async ({ page, cleanup }) => {
  const docId = await seedActivityDoc(page, unique('act-scale'))
...
test('#113 activity rows show a localized event-type label and a verbatim message', async ({ page, cleanup }) => {
  const title = unique('act-label')
  const docId = await seedActivityDoc(page, title)
  cleanup.defer('purge the seeded document', () => deleteDocApi(page.request, docId))
...
test('#113 the client-side event-type filter offers observed types, narrows, and clears', async ({ page, cleanup }) => {
  const docId = await seedActivityDoc(page, unique('act-filter'))
  cleanup.defer('purge the seeded document', () => deleteDocApi(page.request, docId))
  const typeCells = page.locator(`${rowsSelector} .activity-type`)
...
test('#113 the event-type filter can be cleared to all rows with the keyboard', async ({ page, cleanup }) => {
  ...
  const docId = await seedActivityDoc(page, unique('act-kbd'))
  cleanup.defer('purge the seeded document', () => deleteDocApi(page.request, docId))
  const typeCells = page.locator(`${rowsSelector} .activity-type`)
...
test('#113 empty activity response renders the empty state', async ({ page, cleanup }) => {
