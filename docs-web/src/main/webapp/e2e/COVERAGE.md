...
| `admin-guards.spec.ts` | Non-admin routes/redirects are guarded; **disabled-user enforcement (v3.2.2 B)** — an admin disables a user (native login then denied), re-enabling restores it, and the toggle is hidden for the guest+admin rows; **delete affordance (#325)** — those same rows carry no delete control (the backend refuses both outright) while a normal user row does. |
...
| `tag-synonyms.spec.ts` | **Tag synonyms (#280)**: the spec seeds its own tag, its own synonym, its own document and its own second account. A synonym added on the tag edit page SURVIVES A RELOAD (so it is stored, not merely on screen); typing that synonym in the document editor's tag field offers the CANONICAL tag under a label that says why ("Tag (via Synonym)"), and taking it assigns the tag — the chip carries the tag's own name, never the synonym. `tag:<synonym>` then finds the document, exactly as `tag:<name>` does. The second account, which cannot read that tag, resolves the same word to NOTHING: no document, no tag name. A second test pins the collision rule end to end — a synonym that is already another tag's name is warned about while typing and REFUSED at save with the conflicting tag named in the error, and a reload shows nothing was stored. **The swap (TEEDY-153)**: a third test tags a document under the tag's original main name, then uses a synonym chip's "make this the main name" action — the Name field takes the promoted word, the demoted one replaces it among the chips, and the notice names both — and saves through the page's ordinary Save. After a full reload the tree, the edit form and the tag input all show the swapped pair (the input offering the tag as "NewName (via OldName)"), and the document tagged before the swap is still found by BOTH words: the tag was renamed, not replaced. **The split (TEEDY-154)**: a fourth test files a document on the tag, then uses a chip's "split into its own tag" action — the confirmation names both tags and states that the documents stay — and the chip leaves the tag with no Save of its own. After a full reload the tree carries BOTH tags, each with no synonyms, and the document is still found by the original name while the split word (which now resolves to the new, empty tag) finds nothing. |
...
| `comment-editing.spec.ts` | **Editing your own comment (#285)**: admin creates a second account, grants it READ on a document, and both accounts comment. The author edits their own comment (draft seeded with the current text, Save), the wording changes and an "edited" marker appears carrying the edit time in its `title`; the SECOND account then sees both the new wording and the marker on a comment it still has no "Edit comment" control for, while it does have one on its own comment — and editing it works, so the right is authorship, not admin. A cancelled edit is discarded, confirmed after a reload. |
| `access-counters.spec.ts` | **Access counters and their privacy line (#300)**: the spec seeds its own two accounts, its own document and its own file, and grants each account READ. Every "open" is a full reload, because the SPA caches a document detail for 30s and a soft navigation inside that window reaches no server — so it is neither recorded nor an access. The first account's header badge counts ITS OWN opens (1, 2, then 4 after a later one) while the second account independently reaches 3, and the first account's page contains neither that count nor the other username; the aggregate screen is not merely hidden from it, the route bounces a non-admin to the document list. The file panel's per-file badge stays at **0** across those opens (the seeded file is plain text, whose grid card is an icon — rendering the panel fetches none of its bytes), which is the anti-double-count property. The ADMIN then sees the same document ranked on Settings › Statistics with a total spanning both users and a per-user breakdown naming each with their own number. |
