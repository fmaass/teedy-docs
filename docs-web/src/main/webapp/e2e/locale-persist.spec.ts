...
// deferred cleanup step so later specs see the English baseline.
...
test.describe('per-user UI language persistence (#82)', () => {
  ...
  test('language chosen in settings reseeds a fresh device after re-login', async ({ page, cleanup }) => {
    ...
    // Registered before the switch it undoes, and after the login that authorises the POST.
    cleanup.defer('restore the server-side English locale preference', () =>
      page.request.post('/api/user', { form: { locale: 'en' } }),
    )
    cleanup.defer('restore the on-device English locale', () =>
