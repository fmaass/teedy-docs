...
import { ref } from 'vue'
import type { RelationSortDirection, RelationSortField } from '../utils/relationSort'

/** A chosen ordering: which value it reads, and which way it runs. */
...
//
// `null` is the DEFAULT and is not an ordering: it means "leave the server's order alone". The
...
// two disagree. So a comparator runs only after the reader asks for one.
export const useRelationSortStore = defineStore('relationSort', () => {
  const sort = ref<RelationSort | null>(null)
  return { sort }
})
