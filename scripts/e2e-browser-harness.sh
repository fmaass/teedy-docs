...
# 3.3.0). A mismatch FAILS the harness so it can never certify a stale image.
#
# Checks performed (each FAIL exits nonzero at the end):
#   1. /api/app answers and current_version == expected (default 3.3.0)      [api]
...
expect_version="${E2E_EXPECT_VERSION:-3.3.0}"
