...
|---|---|---|---|
| Release identity / version | `smoke.spec.ts` (app shell renders) | **check 1** — `/api/app current_version == 3.2.0` (hard gate) | **Complement** — only E2 asserts the exact release version and fails a stale image. |
...
| Vocabulary dropdown on a document | *(none)* | **check 6** — open the `type` Dublin-Core Select on document-edit, pick a value, save, read back via `/api/document` (authoritative) | **Complement** — E1 has no vocabulary-on-document spec; E2 is the only e2e that drives the vocabulary Select through to a persisted value. |
| Workflow act (start / approve) | *(none)* | **check 7** — start the seeded "Document review" route, ACT on the current step, verify advance via `/api/route` (authoritative) | **Complement** — E1 has no workflow spec; E2 is the only e2e covering a live route transition through the UI. |
