...
public class RoutingUtil {
     ...
     * email (route_step_rejected template) carrying the rejected step's name and, when present,
     * the rejection comment.
     *
     * @param documentId Document ID
     * @param initiatorUserId Initiator user ID
     * @param stepName Name of the rejected step
     * @param comment Rejection comment (may be null)
     */
    public static void sendRouteRejectedEmail(String documentId, String initiatorUserId, String stepName, String comment) {
        DocumentDao documentDao = new DocumentDao();
        Document document = documentDao.getById(documentId);

        UserDao userDao = new UserDao();
        List<UserDto> userDtoList = userDao.findByCriteria(new UserCriteria().setUserId(initiatorUserId), null);
        for (UserDto userDto : userDtoList) {
            RouteStepRejectedEvent routeStepRejectedEvent = new RouteStepRejectedEvent()
                    .setUser(userDto)
                    .setDocument(document)
                    .setStepName(stepName)
                    .setComment(comment);
            AppContext.getInstance().getMailEventBus().post(routeStepRejectedEvent);
        }
    }
