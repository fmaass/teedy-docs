...
public class RouteStepDao {
    ...
    public List<RouteStepDto> findByCriteria(RouteStepCriteria criteria, SortCriteria sortCriteria) {
        ...
        if (criteria.getEndDateIsNull() != null) {
            ...
            // A "current step" query only makes sense for an active route: a route that was
            // halted (REJECTED/CANCELLED) or completed (DONE) has no current step even if an
            // open step row still exists. History queries (endDateIsNull unset) must NOT filter
            // on status so completed/halted routes remain listable.
            if (criteria.getEndDateIsNull()) {
                criteriaList.add("r.RTE_STATUS_C = :routeStatusActive");
                parameterMap.put("routeStatusActive", RouteStatus.ACTIVE.name());
            }
        }
     ...
     * End a route step.
     * <p>
     * Guarded: only an open step (RTP_ENDDATE_D is null) is updated, so a duplicate/racing
     * validation cannot overwrite the transition/comment of an already-closed step.
     *
     * @param id Route step ID
     * @param transition Transition
     * @param comment Comment
     * @param validatorUserId Validator user ID
     * @return Number of rows updated (1 if the step was open, 0 if it was already closed)
     */
    public int endRouteStep(String id, RouteStepTransition transition, String comment, String validatorUserId) {
        ...
        sb.append(" where r.RTP_ID_C = :id and r.RTP_ENDDATE_D is null");
